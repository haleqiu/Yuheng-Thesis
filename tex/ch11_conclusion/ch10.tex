...
This thesis systematically addresses the seven research questions posed in the introduction, organized across Parts I--IV:

\textbf{RQ1--RQ2} (Part~I, Vision Uncertainty and Learning-based Visual Odometry): Demonstrated that learned metrics-aware visual covariance outperforms hand-tuned models for correspondence selection and residual weighting in stereo VO (RQ1), and that sim-to-real calibration with kurtosis-guided quantization enables real-time edge deployment while preserving uncertainty quality (RQ2).

\textbf{RQ3--RQ4} (Part~II, Inertial Uncertainty and Learning-based Inertial Odometry): Showed that data-driven IMU uncertainty and observability-aware body-frame representations enable robust inertial odometry across platforms and dynamics.

\textbf{RQ5--RQ6} (Part~III, Sensor Fusion and Adaptation with Metrics-Aware Uncertainty): Established uncertainty as the foundation for principled visual-inertial fusion (MACVIO) and self-supervised inertial adaptation on real data (MAC-IO).

\textbf{RQ7} (Part~IV, Full Uncertainty-Aware Spatial Perception): Extended uncertainty modeling to multi-frame SLAM with learned covariance for data association, loop closure, and global consistency (MACSLAM).
