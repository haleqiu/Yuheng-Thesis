...
The main chapter reports representative subsets of each benchmark to keep the quantitative discussion readable.
This appendix provides the complementary trajectories that were omitted from the main tables.
\tref{tab:EuRoCEven} completes the EuRoC evaluation by reporting the even-indexed trajectories, while the main text reports the odd-indexed trajectories and the average over the full benchmark.
\tref{tab: TartanAirEasy} reports the Easy split of TartanAir v2, complementing the Hard split used in the main comparison.
\tref{tab: KITTI Odd} provides the remaining KITTI odometry sequences.
Together, these tables show that the conclusions in the main chapter are not driven by a small subset of trajectories.
MAC-VO remains competitive across indoor MAV motion, synthetic large-scale motion, and outdoor driving, and the stereo uncertainty model consistently improves the visual odometry rows without relying on scale alignment.

\input{tex/ch3_macvo/Table/EuRoC_even}

\input{tex/ch3_macvo/Table/TartanAir2_Easy}

\tref{tab: KITTI Odd} gives the complementary KITTI odometry sequences that are not shown in the main chapter.
KITTI differs from EuRoC and TartanAir because the camera motion is dominated by long forward driving with larger scene depth and less frequent viewpoint reversal.
This setting stresses scale consistency and long-baseline stereo geometry rather than short indoor or synthetic agile motion.
The table therefore serves as a cross-domain check: MAC-VO should not only improve on MAV-like trajectories, but should also remain stable on outdoor driving sequences where classical stereo SLAM and learned VO methods have different failure modes.
Reporting the odd KITTI sequences here, together with the even sequences in the main chapter, shows whether the trend holds across the full odometry split instead of a selected subset.

\input{tex/ch3_macvo/Table/KITTI_odd}

\subsection{Robustness Analysis}

See \tref{tab: robustness} for data.

\input{tex/ch3_macvo/Table/RobustnessTartanAir2Hard}
...
\input{tex/ch3_macvo/Table/Ablation_Hard}
\input{tex/ch3_macvo/Table/Ablation_Easy}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{tex/ch3_macvo/figs/Depth_MonteCarlo.pdf}
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{tex/ch3_macvo/figs/DepthPatchEstimate.pdf}
